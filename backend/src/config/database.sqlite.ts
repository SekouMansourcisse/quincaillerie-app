...
// Fonction pour convertir les paramètres pour SQLite
function convertParams(params?: any[]): any[] {
  if (!params) return [];
  return params.map(param => {
    // Convertir les booléens en entiers (SQLite ne supporte pas les booléens)
...
const pool = {
  query: async (text: string, params?: any[]): Promise<{ rows: any[]; rowCount: number }> => {
    try {
      // Convertir les paramètres pour SQLite
      const sqliteParams = convertParams(params);

      // Convertir les paramètres PostgreSQL ($1, $2) en SQLite (?)
      let sqliteQuery = text;
      if (sqliteParams && sqliteParams.length > 0) {
      ...
      // Supprimer RETURNING et tout ce qui suit jusqu'à la fin ou un point-virgule
      ...
      if (isSelect) {
        ...
        const stmt = db.prepare(sqliteQuery);
        const result = sqliteParams.length > 0 ? stmt.run(...sqliteParams) : stmt.run();
        ...
        if (hasReturning && tableName) {
          ...
          if (whereMatch) {
            ...
            const whereParams = sqliteParams.slice(-whereParamCount);
